...
namespace logger {
    ...
    void setBBL(bool b, FILE *out=stdout) {_bbl = b; _bout = out;}
    void setBBLtrace(bool b, FILE *out=stdout) {_bbltrace = b; _btout = out;}

    void BBLtrace(const char *fmt, ...) {
        if (!_bbltrace) return;
        va_list arg;
        va_start(arg, fmt);
        vfprintf(_btout, fmt, arg);
        va_end(arg);
        if (config::isFlush) fflush(_btout);
    }
    ...
    void BBL(const char *fmt, ...) {
    if (!_bbl) return;
    va_list arg;
    va_start(arg, fmt);
    vfprintf(_bout, fmt, arg);
    va_end(arg);
    if (config::isFlush) fflush(_bout);
}
...
namespace filter {
    ...
    bool read(int fd, uint64_t addr, ssize_t size) {
        // logger::info("fd: %d, addr: %lx, size: %lx\n", fd, addr, size);

        bool ret = (fd <= 2 || fd > 10 || addr <= 0x100 || size <= 0 || size >= 131073) // default pass rule:size >=1024
